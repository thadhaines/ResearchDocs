...
\begin{document}
	\begin{tikzpicture}[auto, node distance=1cm,>=triangle 45]
		...
		% delay block w
		\node [block, above=of sum1,label=17:*] (delay1) {$\dfrac{e^{-\$D_A}}{1+\$T_A}$};
		...
		% delay block Pref
		\node [block, above=of sumP,label=17:*] (delay2) {$\dfrac{e^{-\$D_B}}{1+\$T_B}$};
		...
		% w and pref in
		\node [input, name=omega, above= of delay1,label={[label distance=.1cm]0:$\omega_{PU}$} ] {};
