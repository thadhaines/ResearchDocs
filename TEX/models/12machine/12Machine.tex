...
\begin{document}
	...
	\begin{tikzpicture} [american voltages,scale=.8, 
		...
		\coordinate (bus10) at ($(bus1)!0.5!(bus2)+(\csize,0)$)  {};
		...
		% bus 15 gens
		\coordinate (bus9) at ($(bus15)+(\csize,+2.8)$)  {};		
		\coordinate[below = of bus9](bus8) {}; 
		\coordinate[below = of bus8](bus7) {}; 
		\coordinate[below = of bus7](bus6) {}; 
		
		\node[gen, right=0.5cm of bus9] (G9) {};
		\node[gen, right=0.5cm of bus8] (G8) {};
		\node[gen, right=0.5cm of bus7] (G7) {};
		\node[gen, right=0.5cm of bus6] (G6) {};
		
		% Second row
		\coordinate (bus3) at ( $(bus10)!0.5!(bus11)+(0,-2.5)$ ){};
		\coordinate (bus4) at ( $(bus14)!0.3!(bus13)+(0,-2.5)$ ){};
		\coordinate (bus5) at ( $(bus14)!0.3!(bus15)+(0,-2.5)$ ){};
		
		% bus 3 gens
		\node[gen] (G31) at ($ (-1.95 ,-1.2 ) + (bus3)$) {};
		\node[gen] (G32) at ($ (-.65,-1.2 ) + (bus3)$) {};
		\node[gen] (G33) at ($ (+.65,-1.2 ) + (bus3)$) {};
		\node[gen] (G34) at ($ (+1.95,-1.2 ) + (bus3)$) {};
		% area 2 lower gens
		\node[gen] (G5) at ($ (0,-1.2 ) + (bus5)$) {};
		\node[gen] (G4) at ($ (0,-1.2 ) + (bus4)$) {};
		
		% xfmrs straight
		\draw (bus1) to [voosource]  ($ (bus1) + (\csize,0)$){};
		\draw (bus2) to [voosource]  ($ (bus2) + (\csize,0)$){};
		
		\draw (bus9) to [voosource]  ($ (bus9) + (-\csize,0)$){};
		\draw (bus8) to [voosource]  ($ (bus8) + (-\csize,0)$){};
		\draw (bus7) to [voosource]  ($ (bus7) + (-\csize,0)$){};
		\draw (bus6) to [voosource]  ($ (bus6) + (-\csize,0)$){};
		% xfmrs, non staight
		\newcommand{\xfmrOff}{.5}
		\draw ($(bus11)+(0,-\xfmrOff)$) -- ($(bus11)!0.5!(bus10)+(0,-\xfmrOff)$) to [voosource] (bus3){};
		\draw ($(bus14)+(0,-\xfmrOff)$) -- ($(bus14)!0.3!(bus13)+(0,-\xfmrOff)$) to [voosource] (bus4){};
		\draw ($(bus14)+(0,-\xfmrOff)$) -- ($(bus14)!0.3!(bus15)+(0,-\xfmrOff)$) to [voosource] (bus5){};
		
		% load coordinates
		\coordinate (load1) at ($ (-.5,-2 ) + (bus12)$) {};
		\coordinate (load2) at ($ (.5,-2 ) + (bus13)$) {};
		
		% Cap coordinates
		\coordinate (cap1) at ($ (+.5,-1.6 ) + (bus12)$) {};
		\coordinate (cap2) at ($ (-.5,-1.6 ) + (bus13)$) {};
		\coordinate (cap3) at ($ (1.2,-1.6 ) + (bus12)$) {};
		\coordinate (cap4) at ($ (-1.2,-1.6 ) + (bus13)$) {};
		
		% caps are scaled, math calcs used for positioning, pretty complicated - could be made simplier by using more defined styles
		\draw ($ (0,-.6) + (bus13)$) -| (cap2) [/tikz/circuitikz/bipoles/length=.6cm, C] to ($(cap2)+(0,-.2)$) node[/tikz/circuitikz/bipoles/length=.7cm,ground] {}; 
		
		\draw ($ (0,-.6) + (bus12)$) -| (cap1) [/tikz/circuitikz/bipoles/length=.6cm, C] to ($(cap1)+(0,-.2)$) node[/tikz/circuitikz/bipoles/length=.7cm,ground] {}; 
		
		\draw ($ (0,-.6) + (bus12)$) -- ($ (0,-.6) + (bus12)+(1.2,0)$)  [cSZ, nos] to (cap3) [/tikz/circuitikz/bipoles/length=.6cm, C] to ($(cap3)+(0,-.2)$) node[/tikz/circuitikz/bipoles/length=.7cm,ground] {}; 
		
		\draw ($ (0,-.6) + (bus13)$) -- ($ (0,-.6) + (bus13)+(-1.2,0)$)  [cSZ, nos] to  (cap4) [/tikz/circuitikz/bipoles/length=.6cm, C] to ($(cap4)+(0,-.2)$) node[/tikz/circuitikz/bipoles/length=.7cm,ground] {}; 
		
		% drawing of loads
		% shifting here is of the connection on the bus.
		\draw[load] ($(bus12)+(0,-.6)$) -| (load1) ;
		\draw[load] ($(bus13)+(0,-.6)$) -| (load2) ;
		\draw ($(bus12)+(0,-.6)$) -| (cap1) ;
		
		% Vertical draw buses
		\foreach \busNum in {1,2, 6, 7, 8, 9, 11, 12, 13, 14}
		\draw[ultra thick] ([yshift=-0.8cm]bus\busNum.south) -- ([yshift=+0.8cm]bus\busNum.north);
		% Long vertical draw buses
		\foreach \busNum in {10,15}
		\draw[ultra thick] ([yshift=-1.6cm]bus\busNum.south) -- ([yshift=+1.6cm]bus\busNum.north);
		% Very Long vertical draw buses
		\foreach \busNum in {15}
		\draw[ultra thick] ([yshift=-3.5cm]bus\busNum.south) -- ([yshift=+3.5cm]bus\busNum.north);
		...
		\foreach \busNum in {3}
		\draw[ultra thick] ([xshift=-2.5cm]bus\busNum.west) -- ([xshift=+2.5cm]bus\busNum.east);
		
		% Area Split Line
		\coordinate (areaSplitTop) at at ( $(bus12)!0.5!(bus13)+(0,3.6)$ ){};
		\coordinate (areaSplitBtm) at at ( $(bus12)!0.5!(bus13)+(0,-5.3)$ ){};
		...
		\foreach \a/\b in {10/11, 11/12, 12/13, 13/14, 14/15}
		\draw (bus\a) -- (bus\b);
		% horz lines between gens and buses
		\foreach \a in {1,2,4,5,6,7,8,9}
		\draw (G\a) -- (bus\a);
		% verticle degree lines between gens and buses
		\foreach \a/\b in {31/3, 32/3, 33/3, 34/3}
		\draw (G\a) |- (bus\b);
		% tripple lines
		\foreach \a/\b in {12/13}{
		...
		%bus labels right
		\foreach \busNum in {5}
		\draw ($(bus\busNum) +(.8,0)$) node {\large{\busNum}};
		%bus labels Left
		\foreach \busNum in {4}
		\draw ($(bus\busNum) +(-.8,0)$) node {\large{\busNum}};
		%bus labels right above
		\foreach \busNum in {10, 11, 9,8,7,6}
		\draw ($(bus\busNum) +(.4,.3)$) node {\large{\busNum}};
		%bus labels left above
		\foreach \busNum in {1,2,14,15}
		\draw ($(bus\busNum) +(-.4,.3)$) node {\large{\busNum}};
		%bus labels above
		\foreach \busNum in {12,13}
		\draw ($(bus\busNum) +(0,+1.3)$) node {\large{\busNum}};
		
		% singular bus 3 label
		\draw ($(bus3) +(.6,.4)$) node {\large{3}};
		
		% gen labels
		\foreach \genNum in{9,8,7,6} % right
		\draw ($(G\genNum)+ (1.1,0)$) node (G\genNum){\large G\genNum};
		\foreach \genNum in{1,2} % left
		\draw ($(G\genNum)+ (-1.1,0)$) node (G\genNum){\large G\genNum};
		\foreach \genNum in{4,5} % below
		\draw ($(G\genNum)+ (0,-1.1)$) node (G\genNum){\large G\genNum};
		
		% gen labels on same bus
		\draw ($(G31)+ (-.5,-1.1)$) node (G31){\large G3 `1'};
		\draw ($(G32)+ (0,-1.1)$) node (G32){\large  `2'};
		\draw ($(G33)+ (0,-1.1)$) node (G32){\large  `3'};
		\draw ($(G34)+ (0,-1.1)$) node (G32){\large  `4'};
