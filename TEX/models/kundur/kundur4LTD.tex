...
\tikzset{
  ...
  cap lines/.style={do path full={    
    \draw [black, opacity=1, ultra thick] (-1,0) -- (1,0);
    \draw [black, opacity=1, ultra thick] (-1,-1) -- (1,-1);
  }},
}


\tikzset{
	cappy/.style = {rectangle, draw=black, 
		minimum width=.5cm, minimum height=.1cm, cap lines
		},% end of path picture
	}
...
\begin{document}
	...
	\begin{tikzpicture} [american voltages,scale=.8, 
		...
		%\coordinate[right = of bus1](bus5) {};
		%\coordinate[right = of bus5](bus6) {};
		%\coordinate[right = of bus6](bus7) {};
		%\coordinate[right = of bus7](bus8) {};
		%\coordinate[right = of bus8](bus9) {};
		%\coordinate[right = of bus9](bus10) {};
		%\coordinate[right = of bus10](bus11) {};
		%\coordinate[right = of bus11](bus3) {};
		...
		% Cap coordinates
		\coordinate (cap2) at ($ (-.5,-1.5 ) + (bus9)$) {};
		\draw ($ (0,-.6) + (bus9)$) -| (cap2) [/tikz/circuitikz/bipoles/length=.6cm, C] to ($(cap2)+(0,-.25)$) node[/tikz/circuitikz/bipoles/length=.7cm,ground] {}; 
		\node[cappy] at ($ (.5,-1.5 ) + (bus7)$) (cap1) {};
