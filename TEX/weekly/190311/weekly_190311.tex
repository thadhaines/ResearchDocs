...
\begin{document}
	...
	\begin{enumerate}
		...
		\item A more realistic AMQP code process has been written and tested. This includes a more 'batch' style of message communication which would mimic process A sending many messages to process B before handing off control to process B. The coded situation shows the naturally parallel processing advantage of AMQP. 
		...
		\item Terminal output reduced for non-debug runs.
...
\paragraph{Current Tasks:}
	\begin{enumerate}
		\item Continue refactor and incorporation of AMQP into simulation process.
			\subitem Flow chart various processes to more clearly define where, and how, communication can be coded efficiently.
		\item Experiment with ODE solver in numpy / scipy \verb|odeint| in Python 3
			\subitem Make a tgov1 for LTD simulation
